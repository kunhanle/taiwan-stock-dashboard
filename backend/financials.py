...
async def get_stock_info(stock_id: str):
    ...
    def _sync():
        ...
        df = fd.get("company_basic_info").set_index("stock_id")
        name = df.loc[stock_id]["公司簡稱"] if stock_id in df.index else None
        return {"stock_id": stock_id, "name": name}

    try:
        result = await asyncio.to_thread(_sync)
    except Exception:
        result = {"stock_id": stock_id, "name": None}
...
# ─── 6. Contract Liabilities (FinLab 合約負債 + 營業收入淨額) ────────────────
@router.get("/{stock_id}/contract-liabilities")
async def get_contract_liabilities(stock_id: str, years: int = 5):
    cache_key = f"contract_liab|{stock_id}"
    with _lock:
        cached = _cache.get(cache_key)
    if cached and time.time() - cached[0] < CACHE_TTL:
        cl_q, rev_q = cached[1]
        ...
        def _sync():
            ...
            def _scol(df):
                ...
                return pd.to_numeric(df[stock_id], errors="coerce")

            curr  = fd.get("financial_statement:合約負債_流動")
            ncurr = fd.get("financial_statement:合約負債_非流動")
            rev_df = fd.get("financial_statement:營業收入淨額")

            cl = _scol(curr).add(_scol(ncurr), fill_value=0)
            cl.index = pd.to_datetime(cl.index)
            cl = cl[cl > 0].dropna()
            cl_r = cl.reset_index()
            cl_r.columns = ["dt", "cl"]
            cl_r["quarter"] = cl_r["dt"].dt.to_period("Q").astype(str)
            cl_q = cl_r.groupby("quarter")["cl"].last().reset_index()

            rev = _scol(rev_df).dropna()
            rev.index = pd.to_datetime(rev.index)
            rev_r = rev.reset_index()
            rev_r.columns = ["dt", "rev"]
            rev_r["quarter"] = rev_r["dt"].dt.to_period("Q").astype(str)
            rev_q = rev_r.groupby("quarter")["rev"].last().reset_index()

            return cl_q, rev_q

        try:
            cl_q, rev_q = await asyncio.to_thread(_sync)
            with _lock:
                _cache[cache_key] = (time.time(), (cl_q, rev_q))
        except Exception as e:
            return {"data": [], "error": f"FinLab 讀取失敗: {e}"}

    merged = cl_q.merge(rev_q, on="quarter", how="left")
    merged["cl_bn"] = (merged["cl"] / 1e5).round(2)
    merged["ratio"] = (merged["cl"] / merged["rev"] * 100).where(merged["rev"] > 0).round(2)

    cutoff = (datetime.now() - timedelta(days=years * 365)).strftime("%Y") + "Q1"
    result = merged[merged["quarter"] >= cutoff][["quarter", "cl_bn", "ratio"]].dropna(subset=["cl_bn"])
    return {"data": result.rename(columns={"quarter": "date"}).to_dict("records")}


# ─── 7. Major Shareholders (FinLab TDCC 集保股權分散表) ───────────────────────
_LEVEL_LOWER = {
...
async def get_shareholders(stock_id: str, years: int = 5, min_shares: int = 400):
    ...
    cache_key = f"finlab_sh_weekly|{stock_id}|{min(levels)}"
    with _lock:
    ...
    if cached_sh and time.time() - cached_sh[0] < CACHE_TTL:
        ...
        def _sync_fetch():
            ...
            # Sum qualifying levels for each weekly snapshot date
            result = f.groupby("dt")["占集保庫存數比例"].sum().reset_index()
    ...
    sh_filtered = sh_df[sh_df["dt"] >= cutoff].copy()

    # Match each weekly date to nearest trading day price
    price_raw = await _fetch("TaiwanStockPrice", stock_id, start)
    if price_raw:
        price_df = pd.DataFrame(price_raw)[["date", "close"]].copy()
        price_df["dt"] = pd.to_datetime(price_df["date"])
        price_df = price_df[["dt", "close"]].rename(columns={"close": "price"}).sort_values("dt")
        merged = pd.merge_asof(sh_filtered.sort_values("dt"), price_df, on="dt", direction="nearest")
    else:
        merged = sh_filtered.copy()
        merged["price"] = None

    merged["date"] = merged["dt"].dt.strftime("%Y-%m-%d")
    return {"data": merged[["date", "value", "price"]].to_dict("records")}
